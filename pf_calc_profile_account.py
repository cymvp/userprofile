...
def consist_of_foreign_data(key_list, foreign_tuple, collection_name, old_linked_node_map):
    ...
    if old_linked_node is not None and old_linked_node.get('linked_list') is not None:
        if app_id == '1' and metric_id == '0x04':
    ...
    for uid in uidMap:
        ...
        for appIdMetricId in uidMap[uid]:
            ...
            if dataMap is not None and len(dataMap) > 0:
                ...
                for accountId in dataMap:       
                    if resultMap.get(accountId) is None:
                        resultMap[accountId] = dataMap[accountId]
                        resultMap[accountId]['devices'] = []
                    
                    resultMap[accountId]['devices'].append(uid)
    #unique deviceid.
    for accountId in resultMap:
        resultMap[accountId]['devices'] = list(set(resultMap[accountId]['devices']))
                '''
...
if __name__ == "__main__":  
    ...
    for uid in resultMap:
        ...
        for account_id in resultMap[uid]:
            ...
            old_linked_node_map = PFCollectionManager.final_get_linked_node_by_cur(uidMap[account_id])
    ...
    tagMap, userInfoMap = util.calc_tag.calc_tags(userManager,  tagLst)
