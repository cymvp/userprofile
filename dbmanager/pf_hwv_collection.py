...
class PFHWVCollectionManager(PFCollectionManager):
    ...
    def __get_stat_doc(self):
        #初始时;
        _id = 'stat'
        if PFHWVCollectionManager.stat_doc is None:     
            c = self.isDocExist(_id)
            #不存在,则创建一个,并赋值给stat_doc
            if c is None or c.count() == 0:
                return None
            else:     
                PFHWVCollectionManager.stat_doc = next(c.__iter__())        
        return PFHWVCollectionManager.stat_doc 
    ...
    def __update_stat_doc(self, m):
        try:
            ...
            PFHWVCollectionManager.stat_doc = m
