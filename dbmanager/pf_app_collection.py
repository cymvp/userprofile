...
class PFAPPCollectionManager(PFCollectionManager):
    ...
    def __get_stat_doc(self):
        #初始时;
        _id = 'stat'
        if PFAPPCollectionManager.stat_doc is None:     
            c = self.isDocExist(_id)
            #不存在,则创建一个,并赋值给stat_doc
            if c is None or c.count() == 0:
                return None
            else:     
                PFAPPCollectionManager.stat_doc = next(c.__iter__())        
        return PFAPPCollectionManager.stat_doc 
    ...
    def __update_stat_doc(self, m):
        try:
            ...
            PFAPPCollectionManager.stat_doc = m
