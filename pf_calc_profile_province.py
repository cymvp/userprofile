...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Error params...... python pf_calc_profile.py nDays, mTop, ./config/metric_map_province.txt")
        sys.exit(1)
    ...
    for uid in resultMap:
        ...
        for _id in list(userInfoMap[uid]['foreign_key_list'].values())[0]:
            ...
            if len(foreign_data_map) > 0:
                ...
                g_CollectionManager.insertOrUpdateCollection(_id, resultMap[uid])
    ...
    tagMap, userInfoMap = util.calc_tag.calc_tags(g_CollectionManager,  tagLst)
    
    for _id in tagMap:
        profileMap = {PFProvinceCollectionManager.final_getProfileTagLabel():tagMap[_id]}
        g_CollectionManager.insertOrUpdateCollection(_id,  profileMap)
